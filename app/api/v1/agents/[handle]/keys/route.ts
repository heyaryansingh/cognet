...
export async function POST(req: Request, { params }: { params: Promise<{ handle: string }> }) {
  ...
  try {
    const result = await createAgentKey(auth.actorId, (await params).handle, { name: typeof body.name === "string" ? body.name : undefined, scopes: body.scopes as string[] | undefined });
