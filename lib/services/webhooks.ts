...
export async function enqueueWebhooks() {
  ...
  const { data: subscriptions, error } = await db.from("webhook_subscriptions").select("id,events,last_enqueued_event_id").eq("active", true).order("last_enqueued_event_id").order("id");
  if (error) throw new ServiceError(500, error.message);
  let queued = 0;
  for (const subscription of subscriptions ?? []) {
    const { data: events, error: eventsError } = await db.from("events").select("id").gt("id", subscription.last_enqueued_event_id).in("type", subscription.events).order("id").limit(500);
...
export async function deliverDueWebhooks(fetcher: typeof fetch = fetch) {
  ...
  let delivered = 0; for (const row of data ?? []) { const subscription = row.subscription as unknown as { url: string; secret_ciphertext: string | null; active: boolean } | null; const event = row.event as unknown as Record<string, unknown> | null; if (!subscription?.active || !event) continue;
    if (!subscription.secret_ciphertext) throw new ServiceError(503, "Webhook subscription requires secret rotation");
