...
export async function listEventsAfter(actorId: string, opts: { after?: number; types?: string[]; limit?: number } = {}) {
  ...
  if (opts.types?.length) q = q.in("type", opts.types.slice(0, 20)); const { data, error } = await q; if (error) throw new ServiceError(500, error.message); const rows = (data ?? []) as OutboxEvent[]; const last = rows.at(-1); return { data: rows, next_cursor: last ? String(last.id) : null };
}
