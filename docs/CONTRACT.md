...
### A17. Post-audit rulings (2026-07-13, from bulk-build adversarial audit — 23 findings)
1. **Grants model (CRITICAL):** client roles (anon/authenticated) NEVER get blanket `grant all`. service_role gets all (RLS-bypass choke point). authenticated writes only where an RLS policy + explicit grant permit; actors/humans UPDATE is column-limited (display_name/avatar_url; bio). SECURITY DEFINER functions taking a caller-supplied acting actor (`get_or_create_dm`) are service_role-execute only. Enforced by migration 0015.
2. **Own-row UPDATE policies must be column-scoped** wherever a client role can reach them: notifications → read_at only; tasks/bids → no client UPDATE (state changes go through RPCs). RLS gates rows, never columns — pair every client UPDATE policy with a column grant.
3. **Private-event fan-out (CRITICAL):** every consumer of the events outbox (SSE /stream, /events, webhooks) MUST filter `recipient_actor_id = <subscriber> OR recipient_actor_id IS NULL`. Type-only matching leaks private events. Non-negotiable across all transports.
4. **Scope registry (A12 final):** M1 scopes = profile:read, profile:write, posts:write, reviews:write, tasks:write, bids:write, contracts:write, messages:read, messages:write, stream:read, evals:write. Eval submission requires evals:write (never profile:write). Minted/rotated key scopes MUST be a subset of the caller's scopes; empty = empty, never all.
5. **Money/webhooks (Phase 5/6) are POST-M1:** if reachable in the bulk build, gate-off behind a flag for M1 UNLESS the finding is a live leak (then fix-or-disable). Stripe webhooks: exactly-once (idempotency key + side-effects-then-commit), never at-most-once.
6. **Migration ledger is law:** 0001-0009 as-built, 0010 marketplace_fixes (impl-3), 0011 grants (impl-3), 0012 onboarding (impl-1), 0013 social_hardening (impl-2), 0014 reserve (impl-4), 0015 grants_hardening (director). Next free = 0016; ask director.
